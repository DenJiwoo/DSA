...
class WorkoutPlanner: # Class that represetns as the UI for the program
     ...
     def create_widgets(self): # Day selection
          ...
          # Display Routine
          self.display_button = tk.Button(self.root, text="Display Weekly Routine", command=self.display_routine)
          self.display_button.pack(pady=10)
          self.routine_text = tk.Text(self.root, height=10, width=50)  # Add this line
          self.routine_text.pack(pady=10)
     ...
     def add_workout(self):
          ...
          if not (workout_name and sets.isdigit() and reps.isdigit() and rest.isdigit()):
              messagebox.showerror("Input Error", "Please fill in all the blank correctly.")
              return
         
          result = self.routine.add_Workouts(day, workout_name, int(sets), int(reps), int(rest))
          if result == "Rest Day":
              messagebox.showerror("Errpr", f"{day} is set as a Rest Day.")
          else:
               messagebox.showinfo("Success", f"Workout added to {day}!")
               
     def  display_routine(self):
          routines = self.routine.display_routine()
          self.routine_text.delete(1.0, tk.END)
          for day, routine in routines.items():
               self.routine_text.insert(tk.END, f"{day}:\n{routine}\n\n")
